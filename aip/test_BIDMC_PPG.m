...
source_directory = '/home/augusto/Escritorio/GIBIO/DataBases/BIDMC/';
res_source_directory = '/home/augusto/Escritorio/GIBIO/Resultados_articulo_AIP/BIDMC_PPG_Results_aip_mal_parametros/';
...
if flag_procesamiento == 0
    ...
    for file_count = 1:numel(archivos)
        ...
        aux_val = load([source_directory,archivos{file_count}(1:end-4),'_reviewed_annotations.mat']);

        ECGw.ECG_annotations = aux_val.manual_1;
        payload.trgt_width = 180e-3;
        ...
        for count = 1:numel(res.series_quality.AnnNames(:,1))
            for sub_count = 1:max_patterns
                if contains(res.series_quality.AnnNames(count,1),aip_patterns(sub_count))
                    ...
                    resultados(sub_count).TN(index,file_count) = FN;
