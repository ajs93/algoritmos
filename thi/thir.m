function [positions_single_lead, position_multilead] = thir(ECG_matrix, ECG_header, ~, payload_in)
    ...
    f2 = 20 / (ECG_header.freq/2);
    [d_num,d_den] = butter(1,[f1 f2],'bandpass');
...
%                end
                ...
                DPI = ones(1,N);
                valaux = cumsum(HHECG(n0+m1+1:n0+m1+N));
                DPI = (DPI ./ valaux) .* p_arr;
...
%                [~,DPI_valleys_loc] = findpeaks(-DPI(refr_period:end));
        ...
        beats = abs(HECG(arr_n0));
        
        % Correcciones de falsos positivos:
        % Obtencion del histograma
        % Percentilo de los maximos de los latidos
        beats_pctl = prctile(beats, 0:10:100);
        % Mediana de la derivada de lo anterior
        median_pctl = median(diff(beats_pctl));
        % Grilla para el histograma
        if max(beats) / median_pctl > 10e3
            % En este caso mejor no eliminar nada
        else
            bin_edges = min(beats):median_pctl:max(beats);
            [h,~] = histcounts(beats, bin_edges);
            h = h';

            % Picos del histograma
            [hist_peaks,hist_peaks_loc] = findpeaks(h);
            % Centro de masa del histograma (promedio pesado)
            hist_mass_center = sum(hist_peaks .* hist_peaks_loc) / sum(hist_peaks);
            % 50% del latido maximo que abarque el 90% del total del area bajo
            % la curva
            index90 = numel(h);
            while sum(h(1:index90)) > 0.9 * sum(h)
                index90 = index90 - 1;
            end
            max_beat = bin_edges(index90 - 1) / 2;
            % Promediado entre el latido maximo y el centro de masa
            threshold = (max_beat + hist_mass_center) / 2;

            indices = find(beats < threshold);
            auxstr = sprintf('Se encontraron %d indices a eliminar de %d indices. Canal = %d.\n', numel(indices), numel(beats), canal);
            disp(auxstr);
            prev_arr = arr_n0;
            arr_n0(indices) = [];
        end
